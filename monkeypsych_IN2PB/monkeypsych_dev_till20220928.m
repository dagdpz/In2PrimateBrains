...
if strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation') || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation_continuous')
    SETTINGS.dyn.memoryBuffer           = single(nan(SETTINGS.bufferSize,13)); % buffer data before writing to file
    counter_BinRiv = 0;
    counter_Phys = 0;
    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            if strcmp(SETTINGS.background_image, 'gratings')
                global VAR
                if VAR.main_task == 1 % To make it continuous 
                    VAR.current_trial = VAR.current_trial + 1;
                    if task.type == 15
                        trial(dyn.trialNumber).fix_colour = NaN;
                        trial(dyn.trialNumber).stimulus = VAR.condition(dyn.trialNumber);
                    end
                    continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber))
                    if exist('dyn.n_eye_fix')
...
%             end
        ...
        case STATE.FIX_ACQ  % fixation acquisition
            if strcmp(SETTINGS.background_image, 'gratings') && task.type ~= 15
                if VAR.main_task == 0 % for debug
                    continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber));
                else
                    continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber));
                end
            ...
            for n_obj=1:dyn.n_eye_fix
                par_temp=trial(dyn.trialNumber).eye.fix(n_obj);             
%                 if strcmp(SETTINGS.background_image, 'gratings')
%                     par_temp_red = par_temp; par_temp_red.x = par_temp.x.red; par_temp_red.y = par_temp.y.red; par_temp_red.color=trial(dyn.trialNumber).eye.fix(n_obj).color_dim_red;
%                     par_temp_blue = par_temp; par_temp_blue.x = par_temp.x.blue; par_temp_blue.y = par_temp.y.blue; par_temp_blue.color=trial(dyn.trialNumber).eye.fix(n_obj).color_dim_blue;
%                     par_eye_red(n_obj) = aux_FillPar(par_temp_red);
%                     par_eye_blue(n_obj) = aux_FillPar(par_temp_blue);
%                 else
%                     par_temp.color=trial(dyn.trialNumber).eye.fix(n_obj).color_dim;
...
%                 end
            ...
            for n_obj=1:dyn.n_hnd_fix
                ...
                if strcmp(SETTINGS.background_image, 'gratings') && task.type ~= 15
                    par_temp.color_red=trial(dyn.trialNumber).hnd.fix(n_obj).color_dim_red;
                    par_temp.color_blue=trial(dyn.trialNumber).hnd.fix(n_obj).color_dim_blue;
                else
            ...
            if strcmp(SETTINGS.background_image, 'gratings')
                if VAR.main_task == 1
                    if VAR.condition(dyn.trialNumber)==1
                        dyn.duration = task.timing.fix_time_to_acquire_eye;
                        par_eye.deg.x = 0;
                        par_eye.deg.y = 0;
                    elseif VAR.condition(dyn.trialNumber)==5
                        dyn.duration = VAR.short_break;
                    elseif VAR.condition(dyn.trialNumber)==6
                        dyn.duration = VAR.long_break;
                    else
            ...
            if task.type == 15
                if success == 0 % save data and update GUI when failure
                    % Write data to file
                    ...
                    dyn.memoryBuffer    = single(nan(SETTINGS.bufferSize,13));    % reset memoryBuffer
                    dyn.counterLine     = 0;
        ...
        case STATE.FIX_HOL %  fixation holding_with_change_of_color
            ...
            for n_obj=1:dyn.n_eye_fix
                ...
                elseif strcmp(SETTINGS.background_image, 'gratings') && task.type ~= 15
                    if strcmp(dyn.fix_colour, 'Red')
                        par_temp.color=trial(dyn.trialNumber).eye.fix(n_obj).color_bright(1,:);
                    elseif strcmp(dyn.fix_colour, 'Blue')
                        par_temp.color=trial(dyn.trialNumber).eye.fix(n_obj).color_bright(2,:);
                    end
                else
            ...
            for n_obj=1:dyn.n_hnd_fix
                ...
                if task.type == 15
                    ...
                    par_eye.deg.y = 0;
                elseif strcmp(SETTINGS.background_image, 'gratings') && task.type ~= 15
                    par_temp.color_red=trial(dyn.trialNumber).hnd.fix(n_obj).color_bright(1,:);
                    par_temp.color_blue=trial(dyn.trialNumber).hnd.fix(n_obj).color_bright(1,:);
                else
...
%             end
            ...
            if exist('percept_switch', 'var')
                if VAR.condition(dyn.trialNumber) == 1
                    ...
                    trial(dyn.trialNumber).manual_success    = 0;
                    trial(dyn.trialNumber).stimulus = VAR.condition(dyn.trialNumber);
                    fprintf('trial %d (BinRiv: %d, Phys: %d ) \n',dyn.trialNumber, success_counter_BinRiv, success_counter_Phys);
                    VAR.current_trial = VAR.current_trial + 1;
                    continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber)); % update background
                    ...
                    if ~isempty(task.custom_conditions)
                        trial(dyn.trialNumber).success = 1;
                        run(task.custom_conditions);
%                         fprintf('task %.1f ctc %d \n',task.type,custom_trial_condition);
                    end
                    % sending trialinfo to TDT
                    send_trialinfo_to_TDT(IO,dyn,fileNumber);
                elseif VAR.condition(dyn.trialNumber) == 2 || VAR.condition(dyn.trialNumber) == 3
                    dyn.memoryBuffer    = SETTINGS.dyn.memoryBuffer; %preallocate memory buffer...
                    dyn.trialNumber     = dyn.trialNumber + 1;
                    dyn.force_trial_end = false;
                    trial(dyn.trialNumber).states       = []; %#ok<*AGROW> % list of states in the trial
...
%                         fprintf('task %.1f ctc %d \n',task.type,custom_trial_condition);
                    ...
                    % sending trialinfo to TDT
                    send_trialinfo_to_TDT(IO,dyn,fileNumber);
                elseif VAR.condition(dyn.trialNumber) == 4 && percept_switch == 1 % allow break of fixation
                    fprintf('waiting fixation... \n');
                    continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber));
                elseif VAR.condition(dyn.trialNumber) == 4 && percept_switch == 0
                    dyn.memoryBuffer    = SETTINGS.dyn.memoryBuffer; %preallocate memory buffer...
                    dyn.trialNumber     = dyn.trialNumber + 1;
                    dyn.force_trial_end = false;
                    trial(dyn.trialNumber).states       = []; %#ok<*AGROW> % list of states in the trial
...
%                         fprintf('task %.1f ctc %d \n',task.type,custom_trial_condition);
            ...
            else % in case before define percept_switch in BIN_RIV_HOL state
                if VAR.condition(dyn.trialNumber) == 1
                    dyn.memoryBuffer    = SETTINGS.dyn.memoryBuffer; %preallocate memory buffer...
                    dyn.trialNumber     = dyn.trialNumber + 1;
                    dyn.force_trial_end = false;
...
%                         fprintf('task %.1f ctc %d \n',task.type,custom_trial_condition);
                    ...
                    % sending trialinfo to TDT
                    send_trialinfo_to_TDT(IO,dyn,fileNumber);
                elseif VAR.condition(dyn.trialNumber) == 4
                    dyn.memoryBuffer    = SETTINGS.dyn.memoryBuffer; %preallocate memory buffer...
                    dyn.trialNumber     = dyn.trialNumber + 1;
                    dyn.force_trial_end = false;
                    trial(dyn.trialNumber).states       = []; %#ok<*AGROW> % list of states in the trial
...
%                         fprintf('task %.1f ctc %d \n',task.type,custom_trial_condition);
            ...
            % Randomisation (overwritten with custom_conditions)
            if task.randomize_effector, % Randomize effector
                if dyn.trialNumber > 1,
                    if trial(dyn.trialNumber-1).success || task.force_effector == 0,
                        task.effector = randsample([0 1 2],1,true,task.randomize_effector_ratio); % Randomize effector
...
%             continuous_gratings(task.screen_width, VAR.condition(dyn.trialNumber));
            ...
            if VAR.condition(dyn.trialNumber) == 4
...
%             end
            ...
            if success == 1 && VAR.condition(dyn.trialNumber) == 4 && (VAR.condition(dyn.trialNumber + 1) == 2 || VAR.condition(dyn.trialNumber + 1) == 3)
                success_counter_BinRiv = success_counter_BinRiv + 1;
            elseif success == 1 && (VAR.condition(dyn.trialNumber) == 2 || VAR.condition(dyn.trialNumber) == 3) && VAR.condition(dyn.trialNumber + 1) == 4
                success_counter_Phys = success_counter_Phys + 1;
            elseif success == 1 && VAR.condition(dyn.trialNumber) == 4 && (VAR.condition(dyn.trialNumber + 1) == 5 || VAR.condition(dyn.trialNumber + 1) == 6)
                success_counter_BinRiv = success_counter_BinRiv + 1;
            elseif success == 1 && (VAR.condition(dyn.trialNumber) == 2 || VAR.condition(dyn.trialNumber) == 3) && (VAR.condition(dyn.trialNumber + 1) == 5 || VAR.condition(dyn.trialNumber + 1) == 6)
                success_counter_Phys = success_counter_Phys + 1;
            end

            if success == 1 && (VAR.condition(dyn.trialNumber + 1) == 5 || VAR.condition(dyn.trialNumber + 1) == 6) % end of a triad (i.e. next state is STATE.success)
                trial(dyn.trialNumber).fix_colour = dyn.fix_colour;
            ...
            elseif success == 0 && VAR.condition(dyn.trialNumber) == 4 % allow break of fixation (i.e. go back to STATE.BIN_RIV_ACQ)
                success = -1;
                percept_switch = 1;
            elseif success == 0 && VAR.condition(dyn.trialNumber) ~= 4 % DON'T allow break of fixation
                dyn.time_spent_exploring = 0;
                dyn.time_same_targets_were_presented_HOL = 0;
                percept_switch = 0; % trial success, go to next trial
                if SETTINGS.GUI,
                    set(0, 'currentfigure', dyn.GUI_fig_handle);
                    cla;
                    if SETTINGS.matlab_version(1)>=2014
...
%             % end of hack
        ...
        case STATE.SUCCESS % success
            ...
            if strcmp(SETTINGS.background_image, 'gratings') && task.type ~= 15
                ...
                fprintf(' Updating offsets %.2f %.2f %s', x_eye, y_eye, dyn.fix_colour );
                r=par_temp.radius;
                if sqrt((par_temp.x.red - x_eye)^2 + (par_temp.y.red - y_eye)^2) < r % red
                    task.eye.offset_x = task.eye.offset_x + (par_temp.x.red - x_eye);
                    task.eye.offset_y = task.eye.offset_y + (par_temp.y.red - y_eye);
                elseif sqrt((par_temp.x.blue - x_eye)^2 + (par_temp.y.blue - y_eye)^2) < r % blue
                    task.eye.offset_x = task.eye.offset_x + (par_temp.x.blue - x_eye);
                    task.eye.offset_y = task.eye.offset_y + (par_temp.y.blue - y_eye);
                end
            end
            ...
            if task.type == 15
%                 trial(dyn.trialNumber).reward_time = 0;
%                 trial(dyn.trialNumber).task.timing.wait_for_reward = 0;
            else
        ...
        case STATE.BIN_RIV_ITI  % ITI
            ...
            trial(dyn.trialNumber).reward_size = 0;
            trial(dyn.trialNumber).success = 1;
            trial(dyn.trialNumber).fix_colour = NaN;
            trial(dyn.trialNumber).stimulus = VAR.condition(dyn.trialNumber);
            fprintf('trial %d \n',dyn.trialNumber);
...
%             trial(dyn.trialNumber).force_small_reward = force_small_reward;
    ...
    elseif task.type == 15 % RS
        if success == 0 && dyn.state~=STATE.FIX_ACQ && dyn.state~=STATE.FIX_HOL && dyn.state~=STATE.CLOSE
            INI_index = find(VAR.condition==1);
            future_INI_index = INI_index(find(INI_index > dyn.trialNumber)); 
            dyn.trialNumber = future_INI_index(1) - 1; % find next trial corrresponding to the next INI state
            VAR.current_trial = dyn.trialNumber;
            dyn.state = STATE.INI_TRI;
        elseif (success == 0 && dyn.state==STATE.FIX_ACQ) || (success == 0 && dyn.state==STATE.FIX_HOL) 
...
switch dyn.state
    ...
    case STATE.BIN_RIV_ITI
        global VAR
        if VAR.condition(dyn.trialNumber) == 5
...
elseif current_state == STATE.CLOSE,
    ...
    switch task.type
        ...
        case 15 % binocular rivalry task, (choose a unique number here :))
            task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.BIN_RIV_ACQ  STATE.BIN_RIV_HOL  STATE.SUCCESS  STATE.BIN_RIV_ITI];
    end
...
while true
    ...
    if strcmp(SETTINGS.background_image, 'gratings')
        ...
        VertKey = KbName('DownArrow');
        HorzKey = KbName('LeftArrow');
        [keyIsDown, press, keyCode] = KbCheck;
        if keyCode(VertKey)==1
            repo_red = 1;
        elseif keyCode(VertKey)==0
            repo_red = 0;
        end
        if keyCode(HorzKey)==1
            repo_blue = 1;
        elseif keyCode(HorzKey)==0
            repo_blue = 0;
        end 
        
        if VAR.main_task == 1
            if VAR.condition(dyn.trialNumber)==2 % F4 calibrates for red in physical switches
                if any(find(keyCode)==115) && isfield(par_eye,'deg')% F4 
%                     fprintf(' Updating offsets %.2f %.2f', x_eye, y_eye );
                    task.eye.offset_x = task.eye.offset_x + (par_eye(1).deg.x(1,:) - x_eye);
                    task.eye.offset_y = task.eye.offset_y + (par_eye(1).deg.y(1,:) - y_eye);
                end
            elseif VAR.condition(dyn.trialNumber)==3 % F4 calibrates for blue in physical switches
                if any(find(keyCode)==115) && isfield(par_eye,'deg')% F4 
%                     fprintf(' Updating offsets %.2f %.2f', x_eye, y_eye );
                    task.eye.offset_x = task.eye.offset_x + (par_eye(1).deg.x(2,:) - x_eye);
                    task.eye.offset_y = task.eye.offset_y + (par_eye(1).deg.y(2,:) - y_eye);
                end
            elseif VAR.condition(dyn.trialNumber)==1 % F4 calibrates for fixation cue
                if any(find(keyCode)==115) && isfield(par_eye,'deg')% F4 
%                     fprintf(' Updating offsets %.2f %.2f', x_eye, y_eye );
...
%         ListenChar(0);
    ...
    if strcmp(SETTINGS.background_image, 'gratings') 
        dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue];
    else
    ...
    if success,
        if task.type ~= 15
            ...
            try
                if  dyn.state~=STATE.BIN_RIV_ACQ && ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
                    trial(dyn.trialNumber).task.(dyn.effector).(dyn.tar_struct)(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).(dyn.fix_struct).x && ...
                    trial(dyn.trialNumber).task.(dyn.effector).(dyn.tar_struct)(dyn.target_selected(dyn.tar_selected_ind)).y == trial(dyn.trialNumber).task.(dyn.effector).(dyn.fix_struct).y && ...
                    GetSecs - tEnterState < dyn.duration % if target_selected is fixation target and time_to_acquire_eye is not over yet
                    dyn.target_selected = [NaN NaN];
    ...
    switch task.effector,
        case 0 % eye
            ...
            for obj = start_obj_eye:n_obj_eye %obj = dyn.target_selected(1) %obj = 1:n_obj_eye Change 20140519
                if task.type == 15
                    if ~aux_IsWithinRadius_binoriv(x_eye, y_eye, par_eye(obj), dyn)
                        if SETTINGS.allowBlinksOnly && ~aux_IsDisplacedDown(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius)
...
%                             dyn.state   = STATE.ABORT;
                    ...
                    elseif aux_IsWithinRadius_binoriv(x_eye, y_eye, par_eye(obj), dyn)
                        tLeaveFixationEye = 0;  % if inside, reset timer
                    end
                else
                    if ~aux_IsWithinRadius(x_eye, y_eye, par_eye(obj))
...
%         ListenChar(0);
    ...
    elseif dyn.state == STATE.BIN_RIV_ITI
        if any(find(keyCode)==27) % Esc
            success = true;
            dyn.previousState = dyn.state;
            dyn.state   = STATE.CLOSE;
        ...
        if strcmp(SETTINGS.background_image, 'gratings')
            ...
            if strcmp(pha, 'fix') || strcmp(pha, 'fi2') || strcmp(pha, 'cue')
                if VAR.main_task == 1
                    ...
                    elseif VAR.condition(dyn.trialNumber)==2
                    ...
                    elseif VAR.condition(dyn.trialNumber)==3
...
switch shape
    ...
    case 'circle'
        if size(par.stimColor,1)==2 %% hand selection targets, 20140603
            if strcmp(SETTINGS.background_image, 'gratings')
                ...
                if VAR.main_task == 1
                    if VAR.condition(VAR.current_trial)==4
                        Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 0 0 0]);
                        Screen('FillOval', SETTINGS.window, par.stimColor(1,:), par.stimRect(:,1));
                        Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [0 0 1 0]);
                        Screen('FillOval', SETTINGS.window, par.stimColor(2,:), par.stimRect(:,2));
                    ...
                    elseif VAR.condition(VAR.current_trial)==3
                        Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [0 0 1 0]);
                        Screen('FillOval', SETTINGS.window, par.stimColor(2,:), par.stimRect(:,2));
                    elseif VAR.condition(VAR.current_trial)==1
                        Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 0]);
                        Screen('FillOval', SETTINGS.window, [255 255 255], [SETTINGS.screen_w_pix/2-VAR.radius_fp SETTINGS.screen_h_pix/2-VAR.radius_fp SETTINGS.screen_w_pix/2+VAR.radius_fp SETTINGS.screen_h_pix/2+VAR.radius_fp]);
                    end
                else
                    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 0 0 0]);
                    Screen('FillOval', SETTINGS.window, par.stimColor(1,:), par.stimRect(:,1));
            ...
            if strcmp(SETTINGS.background_image, 'gratings')
                if par.stimColor == [255 0 0]
                    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 0 0 0]);
                    Screen('FillOval', SETTINGS.window, par.stimColor, par.stimRect);
                elseif par.stimColor == [0 0 255]
                    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [0 0 1 0]);
                    Screen('FillOval', SETTINGS.window, par.stimColor, par.stimRect);
                end
            else
...
if task.type == 15
    for file_index=1:numel(subfolder_content)-1
...
function continuous_gratings(screen_width, condition)
global VAR
global SETTINGS
...
% from cm to px:
VAR.width_pxpercm = SETTINGS.screenSize(3)/screen_width;
...
VAR.image_red = zeros(round(xysize),round(xysize),3); % spatial freq depends on the size of stimulus
VAR.image_blue = zeros(round(xysize),round(xysize),3); % spatial freq depends on the size of stimulus
...
% create gratings
VAR.image_red(:,:,1) = ((max_intensity_red-42.5) + (sin(x+phase_red) + 1)/2*42.5) .* circle; % RMS contrast=35%
VAR.image_blue(:,:,3) = ((max_intensity_blue-105) + (sin(y+phase_blue) + 1)/2*105) .* circle; % RMS contrast=35%
...
elseif condition == 2
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
	red = Screen('MakeTexture', SETTINGS.window, VAR.image_red);
	Screen('DrawTexture', SETTINGS.window, red)
elseif condition == 3
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
	blue = Screen('MakeTexture', SETTINGS.window, VAR.image_blue);
	Screen('DrawTexture', SETTINGS.window, blue)
elseif condition == 4
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
...
if strcmp(SETTINGS.background_image, 'gratings')
    ...
    is_within = 0;
    fix_colour = '';
    if VAR.condition(dyn.trialNumber) == 1 % fixation cue
        switch par.radiusShape
            case 'circle'
                if sqrt(x^2 + y^2) < r
                    is_within = 1;
    ...
    elseif VAR.condition(dyn.trialNumber) == 4 % binocular rivalry
        switch par.radiusShape
            case 'circle'
    ...
    elseif VAR.condition(dyn.trialNumber) == 2 % red
        switch par.radiusShape
            case 'circle'
                if sqrt(((x0(1,1) - x))^2 + (y0(1,1) - y)^2) < r
                    is_within = 1;
    ...
    elseif VAR.condition(dyn.trialNumber) == 3 % blue
        switch par.radiusShape
            case 'circle'
