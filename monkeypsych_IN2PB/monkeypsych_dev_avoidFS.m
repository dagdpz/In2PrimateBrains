function monkeypsych_dev_avoidFS(monkey)
...
if strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation') || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation_continuous') || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_continuous_binoriv')
    global VAR
%     SETTINGS.dyn.memoryBuffer           = [];
    ...
    create_conditions;
    if ~VAR.playMovie
        make_binocular_rivalry_define_contrasts % load definition of contrasts
        
        images_binoriv = dir(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '/*.tif']);
        for index = 1:length(images_binoriv)
            images_binoriv(index).data = imread(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '/' images_binoriv(index).name]);
        end
        
        images_phys_face = dir(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_' num2str(RedImageContrast) '_Object_0/*.tif']);
        for index = 1:length(images_phys_face)
            images_phys_face(index).data = imread(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_' num2str(RedImageContrast) '_Object_0/' images_phys_face(index).name]);
        end
        
        images_phys_obj = dir(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_0_Object_' num2str(CyanImageContrast) '/*.tif']);
        for index = 1:length(images_phys_obj)
            images_phys_obj(index).data = imread(['BinoRiv_stimuli/with_fp/RedContour_'...
            num2str(RedImageContour) '_RedFP_' num2str(RedImageFixPointContrast)...
            '_BlueContour_' num2str(CyanImageContour) '_BlueFP_' num2str(CyanImageFixPointContrast)...
            '/Face_0_Object_' num2str(CyanImageContrast) '/' images_phys_obj(index).name]);
        end
    end
...
if ~strcmp(monkey_name, 'human') == 1
    ListenChar(2);
elseif strcmp(monkey_name, 'human') == 1
%     if VAR.playMovie
%         % Open movie file   
...
%                     run(task.custom_conditions); % task.eye.fix has info about fixation points
%                 end
%             end
            ...
            if dyn.percept_switch == 0
                VAR.current_trial = VAR.current_trial + 1;
%                 if VAR.num_trial_phys ~= VAR.num_trial_bino
                    if VAR.condition(VAR.current_trial-1) == 1 % fix FPs location during a block
                        run(task.custom_conditions);
                    end
%                 end
                        ...
                        if VAR.condition(VAR.current_trial) == 2
                            ...
                            for index = 1:length(images_phys_face)
                                if strcmp(images_phys_face(index).name, file)
                                    VAR.images = images_phys_face(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
                        elseif VAR.condition(VAR.current_trial) == 3
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.tif'];
                            for index = 1:length(images_phys_obj)
                                if strcmp(images_phys_obj(index).name, file)
                                    VAR.images = images_phys_obj(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
                        elseif VAR.condition(VAR.current_trial) == 4
                            ...
                            for index = 1:length(images_binoriv)
                                if strcmp(images_binoriv(index).name, file)
                                    VAR.images = images_binoriv(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
    ...
    %                     aux_draw_GUI_targets(dyn,trial,'hnd','cue',ang)
           ...
           % pseudo_trial for a run without microstimulation
...
%             dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
            ...
            if success == 0
                ...
                trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye.fix]; % Info about fixation points
                trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd.fix];
%                 trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                 trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
                % state base
...
%             % end of hack
            ...
            if  strcmp(SETTINGS.background_image, 'binoriv')
            else
                Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
                displaybackground(SETTINGS,task,dyn)
                Screen(SETTINGS.window,'Flip');
...
%                 trial(dyn.trialNumber).counter=dyn.counterLine; 
            ...
            if task.type == 15  % save trials
                ...
                save([outFilefolder filesep filename '_' num2str(dyn.trialNumber,'%04u')],'SETTINGS','task','current_trial','VAR');
    ...
    elseif task.type == 15 % RS
         trial(dyn.trialNumber).chopping_label = [trial(dyn.trialNumber).chopping_label dyn.percept_switch];
        if dyn.state==STATE.ABORT || dyn.state==STATE.SUCCESS
    ...
    case STATE.BIN_RIV_ACQ
        ...
        switch trial(dyn.trialNumber).effector
            case 0
                ...
                elseif VAR.condition(VAR.current_trial)==7
                    state_duration = task.timing.tar_time_to_acquire_eye_interval;
                end
            case {1,5,6}
                state_duration = task.timing.tar_time_to_acquire_hnd;
            case {2,3,4}
...
while true
    ...
    if strcmp(SETTINGS.background_image, 'binoriv') 
%         dyn.pseudo_memoryBuffer(dyn.counter_onset,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue];
    ...
    % Avoid unexpected flipping
    if GetSecs - tEnterState > 5 && flip_counter == 0
        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
        Screen('Flip', SETTINGS.window);
        flip_counter = flip_counter + 1;
    elseif GetSecs - tEnterState > 10 && flip_counter == 1
        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
        Screen('Flip', SETTINGS.window);
        flip_counter = flip_counter + 1;
    elseif GetSecs - tEnterState > 15 && flip_counter == 2
        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
        Screen('Flip', SETTINGS.window);
        flip_counter = flip_counter + 1;
    end
...
while true
    ...
    if task.type == 15
        dyn.counter_onset = dyn.counter_onset + 1;
%         dyn.pseudo_memoryBuffer(dyn.counter_onset,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,NaN,NaN];
...
global VAR
VAR.num_trial_phys = 4;
VAR.num_trial_bino = 1;
VAR.num_trial_interval = 1;
VAR.num_triad = 12;
...
VAR.current_trial = 0;
VAR.condition = []; % 1=fixation, 2=red, 3=blue, 4=binoriv, 5=rest, 6=long break, 7=interval
% For the physical switches 
VAR.phys_colour = [];
if VAR.num_trial_phys < 2
    VAR.phys_colour = randi([0 1],1,1); 
end
for i=1:VAR.num_trial_phys/2;VAR.phys_colour=horzcat(VAR.phys_colour,0);end % Red
for i=1:VAR.num_trial_phys/2;VAR.phys_colour=horzcat(VAR.phys_colour,1);end % Blue
...
for spb = 1:VAR.num_superblock
    for trd = 1:VAR.num_triad
        ...
        % Main switch 1
        if switch_seed == 0
            for i = 1:numel(VAR.phys_colour)
                if VAR.phys_colour(i) == 0
                    VAR.condition = vertcat(VAR.condition, 2);
                elseif VAR.phys_colour(i) == 1
                    VAR.condition = vertcat(VAR.condition, 3);
                end
            end
        elseif switch_seed == 1
            VAR.condition = vertcat(VAR.condition, 4);
        end
        
        % Interval
        VAR.condition = vertcat(VAR.condition, 7);
