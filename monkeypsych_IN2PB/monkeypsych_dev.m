...
get_setup_dev; %setting pertain to setup-specific DAQ and display params % GUI coordinates
...
%% program flow, etc
SETTINGS.durTrialMax                = 120;           % max trial duration [s]
...
if strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation')...
        || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation_continuous')...
        || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_continuous_binoriv')...
        || strcmp(task.custom_conditions, 'Y:\Projects\Binocular_rivalry\monkeypsych_IN2PB\conditions\Linus/combined_condition_file_continuous_binoriv')
    global VAR
%     SETTINGS.dyn.memoryBuffer           = [];
    if strcmp(monkey_name, 'human')
        SETTINGS.dyn.memoryBuffer           = single(nan(SETTINGS.bufferSize,14)); % buffer data before writing to file
    elseif strcmp(monkey_name, 'Linus')
        SETTINGS.dyn.memoryBuffer           = single(nan(SETTINGS.bufferSize,12)); % buffer data before writing to file
    end
    VAR.current_trial = 0;
    trials_store = struct;
    counter_BinRiv = 0;
    counter_Phys = 0;
    success_counter_BinRiv = 0;
    success_counter_Phys = 0;
    create_conditions;
    if ~VAR.playMovie
        RedImageContrast = 1;
        CyanImageContrast = 1;
        images_binoriv = dir(['BinoRiv_stimuli/with_fp/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '/*.tif']);
        for index = 1:length(images_binoriv)
            images_binoriv(index).data = imread(['BinoRiv_stimuli/with_fp/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '/' images_binoriv(index).name]);
        end
        images_phys_face = dir(['BinoRiv_stimuli/with_fp/Face_' num2str(RedImageContrast) '_Object_0/*.tif']);
        for index = 1:length(images_phys_face)
            images_phys_face(index).data = imread(['BinoRiv_stimuli/with_fp/Face_' num2str(RedImageContrast) '_Object_0/' images_phys_face(index).name]);
        end
        images_phys_obj = dir(['BinoRiv_stimuli/with_fp/Face_0_Object_' num2str(CyanImageContrast) '/*.tif']);
        for index = 1:length(images_phys_obj)
            images_phys_obj(index).data = imread(['BinoRiv_stimuli/with_fp/Face_0_Object_' num2str(CyanImageContrast) '/' images_phys_obj(index).name]);
        end
    end
        
    % Load a sound for the cue % look at BasicSoundOutputDemo
    wavfilename = [ PsychtoolboxRoot 'PsychDemos' filesep 'SoundFiles' filesep 'radar.wav'];
    % Read WAV file from filesystem:
    if SETTINGS.matlab_version(1)>=2020
        [y, freq] = psychwavread(wavfilename);
    else
        [y, freq] = wavread(wavfilename);
    end
    wavedata = y';
    nrchannels = size(wavedata,1); % Number of rows == number of channels..wav'];
    if nrchannels < 2
...
% [SETTINGS.window, SETTINGS.screenSize] = Screen(0, 'Openwindow'); % 3     % put 'window' on complete screen  KK[ ],[],[],[],[],SETTINGS.AntiAlisingValue
...
%     set(gca,'Ylim',[-SETTINGS.screen_lh_deg SETTINGS.screen_uh_deg],'Xlim',[-SETTINGS.screen_w_deg/2 SETTINGS.screen_w_deg/2],'XLimMode','manual','YLimMode','manual');
    set(gca,'Ylim',[-10 10],'Xlim',[-10 10],'XLimMode','manual','YLimMode','manual');
    if SETTINGS.matlab_version(1)>=2014
        dyn.eye_position_handle = line(-1000,-1000,'Color','k','Marker','o','XlimInclude','off','YLimInclude','off');%'background');
...
STATE.BIN_RIV_ACQ                   = 33; % for the binocular rivalry project
STATE.BIN_RIV_HOL                   = 34; % for the binocular rivalry project
STATE.ITI                           = 50;
STATE.BIN_RIV_ITI                           = 51;
STATE.CLOSE                         = 99;

% for the binocular rivalry project
dyn.time_same_targets_were_presented_HOL = 0;
dyn.time_same_targets_were_presented_ACQ = 0;
...
if ~strcmp(monkey_name, 'human') == 1
    ListenChar(2);
elseif strcmp(monkey_name, 'human') == 1 || strcmp(monkey_name, 'Linus') == 1
%     if VAR.playMovie
%         % Open movie file   
%         %   VAR.flipInterval = Screen('GetFlipInterval', SETTINGS.window);
%         VAR.flipInterval = 0.2; 
% 
%         RedImageContrast = 1;
%         CyanImageContrast = 1;
%         movie_binoriv = dir(['BinoRiv_stimuli/with_fp/movie/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '/*.avi']);
%         movie_binoriv_path = fullfile([pwd '/BinoRiv_stimuli/with_fp/movie/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast)]);
%         for index = 1:length(movie_binoriv)
%             movie_binoriv(index).data = Screen('OpenMovie', SETTINGS.window, [movie_binoriv_path '/' movie_binoriv(index).name], [], 0, 1);
%         end
%         movie_phys_face = dir(['BinoRiv_stimuli/with_fp/movie/Face_' num2str(RedImageContrast) '_Object_0/*.avi']);
%         movie_phys_face_path = fullfile([pwd '/BinoRiv_stimuli/with_fp/movie/Face_' num2str(RedImageContrast) '_Object_0']);
%         for index = 1:length(movie_phys_face)
%             movie_phys_face(index).data = Screen('OpenMovie', SETTINGS.window, [movie_phys_face_path '/' movie_phys_face(index).name], [], 0, 1);
%         end
%         movie_phys_obj = dir(['BinoRiv_stimuli/with_fp/movie/Face_0_Object_' num2str(CyanImageContrast) '/*.avi']);
%         movie_phys_obj_path = fullfile([pwd '/BinoRiv_stimuli/with_fp/movie/Face_0_Object_' num2str(CyanImageContrast)]);
%         for index = 1:length(movie_phys_obj)
%             movie_phys_obj(index).data = Screen('OpenMovie', SETTINGS.window, [movie_phys_obj_path '/' movie_phys_obj(index).name], [], 0, 1);
%         end   
%     end
end
%% MAIN LOOP
...
while true
    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            if dyn.trialNumber > 1,
                if task.type == 15 
                    if isempty(trial(dyn.trialNumber-1).choice) || isempty(trial(dyn.trialNumber-1).reward_size) || isempty(trial(dyn.trialNumber-1).success)
                        force_small_reward = 0;
                        task.choice = 0;
            ...
            %% For the binocular rivaly task
            if task.type == 15
                global VAR
                VAR.current_trial = VAR.current_trial + 1;
                dyn.percept_switch = 0;
                dyn.counter_onset = 0;
                
                trial(dyn.trialNumber).chopping_label = [];
                trial(dyn.trialNumber).counter_onset = [];
                trial(dyn.trialNumber).fix_colour = {};
                trial(dyn.trialNumber).stimulus = [];
                
                trial(dyn.trialNumber).eye = [];
                trial(dyn.trialNumber).hnd = [];
                trial(dyn.trialNumber).aborted_state = [];
                trial(dyn.trialNumber).aborted_state_duration = [];
                trial(dyn.trialNumber).abort_code = {};
                trial(dyn.trialNumber).target_selected = [];
                trial(dyn.trialNumber).target2_selected = [];
                trial(dyn.trialNumber).reward_selected = [];
                trial(dyn.trialNumber).reward_prob = [];
                trial(dyn.trialNumber).reward_time = [];
                
                % state base
                trial(dyn.trialNumber).fix_colour = [trial(dyn.trialNumber).fix_colour {'INI'}];
                
                fprintf('trial %d , fixation cue... \n', VAR.current_trial);
                
                task.eye.fix.pos_red=[task.eye.fix.x.red task.eye.fix.y.red task.eye.fix.size task.eye.fix.radius 1]; %% legacy for analysis
                task.eye.fix.pos_blue=[task.eye.fix.x.blue task.eye.fix.y.blue task.eye.fix.size task.eye.fix.radius 1]; %% legacy for analysis
                task.hnd.fix.pos_red=[task.hnd.fix.x.red task.hnd.fix.y.red task.hnd.fix.size task.hnd.fix.radius 1]; %% legacy for analysis
                task.hnd.fix.pos_blue=[task.hnd.fix.x.blue task.hnd.fix.y.blue task.hnd.fix.size task.hnd.fix.radius 1]; %% legacy for analysis
            else
...
%             task.hnd.fi2.pos=[task.hnd.fi2.x task.hnd.fi2.y task.hnd.fi2.size task.hnd.fi2.radius 1]; %% legacy for analysis
            ...
            % IK 20220113 % RS 20220906
%             if task.type == 1 || task.type == 2,
%                 dyn.n_eye_cue=0;
%                 dyn.n_hnd_cue=0;
%             end
            if task.type == 1 || task.type == 2 || task.type == 13 || task.type == 14 || task.type == 15
                dyn.n_eye_cue=0;
                dyn.n_hnd_cue=0;
            end
...
%                 aux_draw_GUI_targets(dyn,trial,eff,pha,ang)
            ...
            if task.type == 14
                task.state_counter = 16;
            end
            
            
        case STATE.FIX_ACQ  % fixation acquisition
            if task.type == 15
                % Sound inducing fixation
                if dyn.percept_switch == 0
...
%                 end
                ...
                if VAR.condition(VAR.current_trial)==1
                    dyn.duration = task.timing.fix_time_to_acquire_eye;
                    par_eye.deg.x = 0;
                    par_eye.deg.y = 0;
                else
                    dyn.duration            = get_state_duration(task,trial,dyn);
                end
                pseudo_trial(dyn.trialNumber).microstim = 0; % to maintain sampling rate
                pseudo_trial(dyn.trialNumber).microstim_state = NaN;
                [success,dyn,task]      = acquisition_state(task,par_eye,par_hnd,dyn,pseudo_trial,IO);
%                 dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
                dyn.time_spent_exploring = dyn.time_spent_exploring + dyn.time_spent_in_state; % storing total time to decide about trial end (in hold state)
                dyn.time_same_targets_were_presented_ACQ = dyn.time_same_targets_were_presented_ACQ + dyn.time_spent_in_state; % storing information for forced target switch           
            ...
            if task.type == 15
                if success == 0
                    ...
                    trial(dyn.trialNumber).stimulus = [trial(dyn.trialNumber).stimulus VAR.condition(VAR.current_trial)];
                    trial(dyn.trialNumber).reward_selected = [trial(dyn.trialNumber).reward_selected {'NaN'}];
                    trial(dyn.trialNumber).reward_prob = [trial(dyn.trialNumber).reward_prob {'NaN'}];
                    trial(dyn.trialNumber).reward_time = [trial(dyn.trialNumber).reward_time {'NaN'}];
                    trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye.fix];
                    trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd.fix];
%                     trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                     trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
                    % state base
                    trial(dyn.trialNumber).fix_colour = [trial(dyn.trialNumber).fix_colour {'NaN'}];
                    
                    if SETTINGS.GUI, % clear GUI
                        set(0, 'currentfigure', dyn.GUI_fig_handle);
                        cla;
                        if SETTINGS.matlab_version(1)>=2014
                            dyn.eye_position_handle = line(0,0,'Color','k','Marker','o','XlimInclude','off','YLimInclude','off');%'background');
                ...
                elseif success == 1
                    % state base
                    trial(dyn.trialNumber).fix_colour = [trial(dyn.trialNumber).fix_colour {'Cue'}];
                end
            end            
        ...
        case STATE.FIX_HOL %  fixation holding_with_change_of_color
            ...
            if task.type == 15
                ...
                dyn.duration            = get_state_duration(task,pseudo_trial,dyn);
%                 dyn.duration = dyn.duration - dyn.time_same_targets_were_presented_HOL; %take into account time that the same (target) stimuli have been presented already
                [success,dyn,task]      = hold_state(task,par_eye,par_hnd,dyn,pseudo_trial,IO);
...
%                 dyn.time_same_targets_were_presented_HOL = dyn.time_same_targets_were_presented_HOL + dyn.time_spent_in_state; % storing information for forced target switch
                trial(dyn.trialNumber).fix_colour = [trial(dyn.trialNumber).fix_colour {'Cue'}];
                
                if success == 1
                    dyn.time_spent_exploring = 0;
                    dyn.time_same_targets_were_presented_HOL = 0;
                    dyn.percept_switch = 0; % trial success, go to next trial
                    trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 1];
                elseif success == 0 % allow break of fixation (i.e. go back to STATE.BIN_RIV_ACQ)
                    success = -1;
                    dyn.percept_switch = 1;
                end
    ...
    %                 trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
                ...
                dyn.duration            = get_state_duration(task,trial,dyn);
                [success,dyn,task]      = hold_state(task,par_eye,par_hnd,dyn,trial,IO);
                if task.type > 1 && task.type~=11 && task.type~=12 && task.type~=13 && task.type~=14, %% not clean solution to provide reward, i.e. have a target selected to chose from
                    dyn.target_selected = [NaN NaN];
                end
            end

            
        case STATE.BIN_RIV_ACQ  % target acquisition in binocular rivalry task (name is obviously exchangeable, assign a UNIQUE number to it in the beginning of monkeypsych)
%             if dyn.it_is_time_to_change_fixation_positions
                % some sort of random selection of 2 new target positions  
                % trial(dyn.trialNumber).eye.tar stores information about targets to be presented
                % potentially adjust dyn.n_eye_tar & dyn.n_hnd_tar 
%                 dyn.time_same_targets_were_presented = 0; %  reset timer
%             end
...
%             task.hnd.fix.pos_blue = [task.hnd.fix.x.blue task.hnd.fix.y.blue task.hnd.fix.size task.hnd.fix.radius 1]; %% legacy for analysis
            ...
            dyn.trial_classifier        = 250;
            if ~isempty(task.custom_conditions)
                if VAR.num_trial_phys == VAR.num_trial_bino
                    run(task.custom_conditions);
                end
            end
            ...
            if dyn.percept_switch == 0
                VAR.current_trial = VAR.current_trial + 1;
                if VAR.num_trial_phys ~= VAR.num_trial_bino
                    if VAR.condition(VAR.current_trial-1) == 1
                        run(task.custom_conditions);
                    end
                end
                
                if VAR.playMovie
                    if VAR.only_binoriv == 0
                        % 1=fixation, 2=red, 3=blue, 4=binoriv, 5=rest, 6=long break
                        %% dynamic stimuli
                        if VAR.condition(VAR.current_trial) == 2
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.avi'];
                            for index = 1:length(movie_phys_face)
                                if strcmp(movie_phys_face(index).name, file)
                                    VAR.movie = movie_phys_face(index).data;
                                    break
                                end
                            end
                            %Start the movie playback
                            Screen('PlayMovie', VAR.movie, 1);
                        elseif VAR.condition(VAR.current_trial) == 3
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.avi'];
                            for index = 1:length(movie_phys_obj)
                                if strcmp(movie_phys_obj(index).name, file)
                                    VAR.movie = movie_phys_obj(index).data;
                                    break
                                end
                            end
                            %Start the movie playback
                            Screen('PlayMovie', VAR.movie, 1);
                        elseif VAR.condition(VAR.current_trial) == 4
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.avi'];
                            for index = 1:length(movie_binoriv)
                                if strcmp(movie_binoriv(index).name, file)
                                    VAR.movie = movie_binoriv(index).data;
                                    break
                                end
                            end
                            %Start the movie playback
                    ...
                    if VAR.only_binoriv == 0
                        if VAR.condition(VAR.current_trial) == 2
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.tif'];
                            for index = 1:length(images_phys_face)
                                if strcmp(images_phys_face(index).name, file)
                                    images = images_phys_face(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), images)
                        elseif VAR.condition(VAR.current_trial) == 3
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.tif'];
                            for index = 1:length(images_phys_obj)
                                if strcmp(images_phys_obj(index).name, file)
                                    images = images_phys_obj(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), images)
                        elseif VAR.condition(VAR.current_trial) == 4
                            file = ['Face_and_Body_binocular_FaceFixPosition' num2str(task.fix_loc_label(1,:)) '_BodyFixPosition' num2str(task.fix_loc_label(2,:)) '.tif'];
                            for index = 1:length(images_binoriv)
                                if strcmp(images_binoriv(index).name, file)
                                    images = images_binoriv(index).data;
                                    break
                                end
                            end
                            binoriv_stimuli(VAR.condition(VAR.current_trial), images)
            ...
            for n_obj=1:dyn.n_eye_fix
                par_temp = task.eye.fix(n_obj);
                par_temp.color = task.eye.fix(n_obj).color_dim;
                par_temp.size  = task.eye.fix.size;
                par_temp.shape = task.eye.fix.shape;
                par_eye(n_obj) = aux_FillPar(par_temp);
            end
           ...
           if dyn.percept_switch == 0
               ...
               if VAR.fix_trial_duration
                   dyn.time_spent_exploring = 0;
                   dyn.duration_origin            = get_state_duration(task,trial,dyn);
               end
               
                trial(dyn.trialNumber).stimulus = [trial(dyn.trialNumber).stimulus VAR.condition(VAR.current_trial)];
                % Draw GUI
                dyn.correct_choice_target = task.correct_choice_target;
                dyn.hnd_color=nanmean([task.hnd.fix.color_bright],1);
                gui(dyn.trialNumber).eye.fix = task.eye.fix;
                if SETTINGS.GUI
                    set(0, 'currentfigure', dyn.GUI_fig_handle);
                    ang=0:0.02:2*pi;
    %                     aux_draw_GUI_targets(dyn,trial,eff,pha,ang)
                    aux_draw_GUI_targets(dyn,gui,'eye','fix',ang) % FPs
    %                     aux_draw_GUI_targets(dyn,trial,'eye','fi2',ang)
    %                     aux_draw_GUI_targets(dyn,trial,'eye','tar',ang)
    %                     aux_draw_GUI_targets(dyn,trial,'eye','ta2',ang)
    %                     aux_draw_GUI_targets(dyn,trial,'eye','cue',ang)
    ...
    %                     aux_draw_GUI_targets(dyn,trial,'hnd','cue',ang)
            ...
            pseudo_trial(dyn.trialNumber).microstim = 0;
            pseudo_trial(dyn.trialNumber).microstim_state = NaN;
            
            % State duration
            if VAR.fix_trial_duration
                % Allow break of fixation in BinoRiv&Phys conditions in ACQ&HOLD state
                dyn.duration = dyn.duration_origin - dyn.time_spent_exploring;
            else
                dyn.duration            = get_state_duration(task,trial,dyn); %duration in acquisition state means allowed time to explore without entering target radius
            end
            [success,dyn,task]      = acquisition_state(task,par_eye,par_hnd,dyn,pseudo_trial,IO);
%             [success,dyn,task]      = acquisition_state(task,par_eye,par_hnd,dyn,trial,IO);
%             dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
            ...
            % if failed to acquire any target within timing.target_aquisition
            % success will be 0 --> state_transition: condition is aborted
%             if dyn.time_spent_exploring > dyn.duration
%                 success = 0;
%             end

            if success == 0
                % trial base
                trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 0];
                trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye.fix];
                trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd.fix];
%                 trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                 trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
            ...
            elseif success == 1
                trial(dyn.trialNumber).fix_colour = [trial(dyn.trialNumber).fix_colour dyn.fix_colour];
            end

            

        case STATE.BIN_RIV_HOL   % target hold in binocular rivalry task (name is obviously exchangeable, assign a UNIQUE number to it in the begionning of monkeypsych)
...
%             par_hnd = struct([]);       
            ...
            % State duration
            if VAR.fix_trial_duration % Allow break of fixation in BinoRiv&Phys conditions in ACQ&HOLD state
                dyn.duration = dyn.duration_origin - dyn.time_spent_exploring;
            else
                dyn.duration             = get_state_duration(task,trial,dyn); % duration of hold state == time between forced target switches!   
                % Allow break of fixation in only BinoRiv condition in HOLD state
%                 if VAR.condition(VAR.current_trial) == 4 
%                     dyn.duration = dyn.duration - dyn.time_same_targets_were_presented_HOL; %take into account time that the same (target) stimuli have been presented already
%                 end
                % Allow break of fixation in BinoRiv&Phys conditions in HOLD state
                dyn.duration = dyn.duration - dyn.time_same_targets_were_presented_HOL; %take into account time that the same (target) stimuli have been presented already
            end
            [success,dyn,task]       = hold_state(task,par_eye,par_hnd,dyn,pseudo_trial,IO);
%             [success,dyn,task]       = hold_state(task,par_eye,par_hnd,dyn,trial,IO);
...
% 
%             %% define if it is time to change fixation positions
%             % success will be 1 IF (at the end of triad) and only if target was looked at for more than dyn.duration (which means force target switch!)
%             dyn.it_is_time_to_change_fixation_positions = success; 
%             dyn.target_selected(1)=NaN;
%             dyn.target_selected(2)=NaN;
%             success = -1;
% 
%             if dyn.time_spent_exploring > task.timing.entire_trial_duration %% define in settings, this is "success" condition, time has run out
%                 trial_success_counter = trial_success_counter + 1; % --> get_state_transition: go to STATE.SUCCESS (ITI, w.e)
%             end
%             
%             if trial_success_counter == 8 * 2;
%                 success = 1;
%                 trial_success_counter = 0;
%             end

            if success == 1 && VAR.condition(VAR.current_trial) == 4 && (VAR.condition(VAR.current_trial + 1) == 2 || VAR.condition(VAR.current_trial + 1) == 3)
                success_counter_BinRiv = success_counter_BinRiv + 1;
            elseif success == 1 && (VAR.condition(VAR.current_trial) == 2 || VAR.condition(VAR.current_trial) == 3) && VAR.condition(VAR.current_trial + 1) == 4
                success_counter_Phys = success_counter_Phys + 1;
            elseif success == 1 && VAR.condition(VAR.current_trial) == 4 && (VAR.condition(VAR.current_trial + 1) == 5 || VAR.condition(VAR.current_trial + 1) == 6)
                success_counter_BinRiv = success_counter_BinRiv + 1;
            elseif success == 1 && (VAR.condition(VAR.current_trial) == 2 || VAR.condition(VAR.current_trial) == 3) && (VAR.condition(VAR.current_trial + 1) == 5 || VAR.condition(VAR.current_trial + 1) == 6)
                success_counter_Phys = success_counter_Phys + 1;
            end

            if success == 1 && VAR.condition(VAR.current_trial + 1) == 5 % end of a triad (i.e. next state is STATE.success)
                dyn.time_spent_exploring = 0;
                dyn.time_same_targets_were_presented_HOL = 0;
                dyn.percept_switch = 0; % trial success, go to next trial
                trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 1];
            elseif success == 1 && VAR.condition(VAR.current_trial + 1) == 6  % end of a triad (i.e. next state is STATE.success)
                dyn.time_spent_exploring = 0;
                dyn.time_same_targets_were_presented_HOL = 0;
                dyn.percept_switch = 0; % trial success, go to next trial
                trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 1];
            elseif success == 0 % allow break of fixation (i.e. go back to STATE.BIN_RIV_ACQ)
                success = -1;
                dyn.percept_switch = 1;
%             elseif success == 0 && VAR.condition(VAR.current_trial) == 4 % allow break of fixation in only BinoRiv condition (i.e. go back to STATE.BIN_RIV_ACQ)
%                 success = -1;
%                 dyn.percept_switch = 1;
%             elseif success == 0 && VAR.condition(VAR.current_trial) ~= 4 % DON'T allow break of fixation in Phys condition
%                 dyn.time_spent_exploring = 0;
%                 dyn.time_same_targets_were_presented_HOL = 0;
%                 dyn.percept_switch = 0; % trial success, go to next trial
%                 trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 0];
            elseif success == 1 && VAR.condition(VAR.current_trial + 1) ~= 5 && VAR.condition(VAR.current_trial + 1) ~= 6
                dyn.time_spent_exploring = 0;
                dyn.time_same_targets_were_presented_HOL = 0;
                success = -1;
                dyn.percept_switch = 0; % trial success, go to next trial
            ...
            if dyn.percept_switch == 0 % store the data and update GUI
%                 if success == 0 && VAR.condition(VAR.current_trial) ~= 4 % allow break of fixation in only BinoRiv condition
%                     trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye.fix];
%                     trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd.fix];
% %                     trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
% %                     trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task];
                if success == 0 % allow break of fixation in BinoRiv&Phys conditions
                    trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye.fix];
                    trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd.fix];
%                     trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                     trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task];
...
%                     trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                     trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
                    trial(dyn.trialNumber).target_selected = [trial(dyn.trialNumber).target_selected dyn.target_selected];
                    trial(dyn.trialNumber).target2_selected = [trial(dyn.trialNumber).target2_selected dyn.target2_selected];
                    trial(dyn.trialNumber).reward_selected = [trial(dyn.trialNumber).reward_selected {'NaN'}];
                    trial(dyn.trialNumber).reward_prob = [trial(dyn.trialNumber).reward_prob {'NaN'}];
                    trial(dyn.trialNumber).reward_time = [trial(dyn.trialNumber).reward_time {'NaN'}];
                end
        ...
        case STATE.ABORT % abort
            ...
            if task.type == 15
                trial(dyn.trialNumber).aborted_state = [trial(dyn.trialNumber).aborted_state dyn.previousState];
                trial(dyn.trialNumber).aborted_state_duration = [trial(dyn.trialNumber).aborted_state_duration dyn.time_spent_in_state];
                trial(dyn.trialNumber).abort_code = [trial(dyn.trialNumber).abort_code dyn.abort_code];
                trial(dyn.trialNumber).target_selected = [trial(dyn.trialNumber).target_selected dyn.target_selected];
                trial(dyn.trialNumber).target2_selected = [trial(dyn.trialNumber).target2_selected dyn.target2_selected];
                trial(dyn.trialNumber).reward_time = [trial(dyn.trialNumber).reward_time 0];
                trial(dyn.trialNumber).reward_prob = [trial(dyn.trialNumber).reward_prob -1];
                trial(dyn.trialNumber).reward_selected = [trial(dyn.trialNumber).reward_selected 0];
                trial(dyn.trialNumber).completed = 0;
...
%             % end of hack
            ...
            if  strcmp(SETTINGS.background_image, 'gratings')
            else
                Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
                displaybackground(SETTINGS,task,dyn)
                Screen(SETTINGS.window,'Flip');
        ...
        case STATE.SUCCESS % success
            ...
            if task.type == 15
                ...
                trial(dyn.trialNumber).abort_code               = dyn.abort_code;
                trial(dyn.trialNumber).completed                = [trial(dyn.trialNumber).completed 1];
                trial(dyn.trialNumber).target_selected          = [trial(dyn.trialNumber).target_selected dyn.target_selected];
                trial(dyn.trialNumber).target2_selected         = [trial(dyn.trialNumber).target2_selected dyn.target2_selected];
            else
                trial(dyn.trialNumber).aborted_state            = -1;
            ...
            if task.type == 15
%                 tar_selected=NaN;
%                 trial(dyn.trialNumber).reward_selected = NaN;
%                 trial(dyn.trialNumber).reward_prob = NaN;
%                 reward_idx = NaN; % take first or secondary reward for specific target
%                 trial(dyn.trialNumber).reward_time = NaN;
            else
            ...
            if task.type == 15
                Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
                Screen('FillRect', SETTINGS.window, [186 186 186]);
                Screen(SETTINGS.window,'Flip');
            else
                Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
                displaybackground(SETTINGS,task,dyn)
        ...
        case STATE.ITI  % ITI
            ...
            if task.type == 15
                VAR.current_trial = VAR.current_trial + 1;
%                 continuous_gratings(task.screen_width, VAR.condition(VAR.current_trial)); % update background
                fprintf('\ntrial %.0f, break... \n',VAR.current_trial);
            end
            ...
            if task.type == 15
                [trial, trials_store, triad_trial] = BinoRiv_ITI_saving(trial,task,dyn,trials_store);
                % dyn.memoryBuffer    = [];    % reset memoryBuffer
                if strcmp(SETTINGS.human_monkey, 'human')
                    dyn.memoryBuffer    = single(nan(SETTINGS.bufferSize,14));    % reset memoryBuffer
                elseif strcmp(SETTINGS.human_monkey, 'monkey')
                    dyn.memoryBuffer    = single(nan(SETTINGS.bufferSize,12));    % reset memoryBuffer
                end
                dyn.counterLine     = 0;
            else
                 %% Write data to file
            ...
            if task.type == 15  % save trials
                for trial_num = (triad_trial+1):numel(trials_store)
                    current_trial = trials_store(trial_num);
                    save([outFilefolder filesep filename '_' num2str(trial_num,'%04u')],'SETTINGS','task','current_trial','VAR');
                end
                if trial(dyn.trialNumber).success(1) == 1
                    trial(dyn.trialNumber).success = 1;
                    trial(dyn.trialNumber).choice = 0;
                else
                    trial(dyn.trialNumber).success = 0;
                    trial(dyn.trialNumber).choice = 0;
                end
        ...
        case STATE.CLOSE % close gracefully
            ...
            if isfield(trial,'state') && ~isempty(trial(1).state)
                ...
                combine_trials_from_single_matfiles(pathname,filename,task);
    ...
    elseif task.type == 15 % RS
         trial(dyn.trialNumber).chopping_label = [trial(dyn.trialNumber).chopping_label dyn.percept_switch];
        if dyn.state==STATE.ABORT
            trial(dyn.trialNumber).counter_onset = [trial(dyn.trialNumber).counter_onset 0];
            trial(dyn.trialNumber).completed = [trial(dyn.trialNumber).completed 0]; 
            dyn.percept_switch = 0;
        else
            trial(dyn.trialNumber).counter_onset = [trial(dyn.trialNumber).counter_onset dyn.counter_onset];
            trial(dyn.trialNumber).completed = [trial(dyn.trialNumber).completed 1]; 
        end
       

        if dyn.state==STATE.ITI
            ITI_index = find(VAR.condition==5 | VAR.condition==6);
            future_ITI_index = ITI_index(find(ITI_index >= VAR.current_trial)); 
%             dyn.trialNumber = future_ITI_index(1) - 1; % find next ITI trial corrresponding to the next INI state
            VAR.current_trial = future_ITI_index(1);
            dyn.state = STATE.INI_TRI;
        elseif success == 0
            if VAR.Skip_IfFailure == 1
                if dyn.state==STATE.FIX_ACQ || dyn.state==STATE.FIX_HOL
                    dyn.state = STATE.BIN_RIV_ACQ;
                end
            elseif VAR.Skip_IfFailure == 0 && dyn.state~=STATE.CLOSE
                dyn.state               = state_transition(task,success,dyn.state);
            end
        elseif dyn.state==STATE.ABORT
            if VAR.Skip_IfFailure == 1
                ITI_index = find(VAR.condition==5 | VAR.condition==6);
                future_ITI_index = ITI_index(find(ITI_index >= VAR.current_trial)); 
    %             dyn.trialNumber = future_ITI_index(1) - 1; % find next ITI trial corrresponding to the next INI state
                VAR.current_trial = future_ITI_index(1)-1;
                dyn.state = STATE.ITI;
            elseif VAR.Skip_IfFailure == 0
                dyn.state               = state_transition(task,success,dyn.state);
            end
        elseif dyn.state==STATE.CLOSE % in case of Esc
        else
            dyn.state               = state_transition(task,success,dyn.state);
        end
    else
...
switch dyn.state
    ...
    case STATE.BIN_RIV_ACQ
        global VAR
        switch trial(dyn.trialNumber).effector
            case 0
                if VAR.condition(VAR.current_trial)==4
                    state_duration = task.timing.tar_time_to_acquire_eye_bino;
                elseif VAR.condition(VAR.current_trial)==2 || VAR.condition(VAR.current_trial)==3
                    state_duration = task.timing.tar_time_to_acquire_eye_phys;
                end
            case {1,5,6}
                state_duration = task.timing.tar_time_to_acquire_hnd;
            case {2,3,4}
    ...
    case STATE.BIN_RIV_HOL
        state_duration = task.timing.tar_time_hold + rand*task.timing.tar_time_hold_var;
        
    case STATE.ITI
        if task.type == 15
            global VAR
            if VAR.condition(VAR.current_trial)==5
                state_duration = VAR.short_break;
            elseif VAR.condition(VAR.current_trial)==6
                state_duration = VAR.long_break;
            end
        else
            if trial(dyn.trialNumber).success
...
elseif current_state == STATE.ABORT && task.type == 15,
    global VAR
    if VAR.Skip_IfFailure == 0
        if VAR.condition(VAR.current_trial+1) == 5 ||  VAR.condition(VAR.current_trial+1) == 6
            new_state = STATE.ITI;
        else
            new_state = STATE.BIN_RIV_ACQ;
        end
    end
...
elseif current_state == STATE.BIN_RIV_ITI
    new_state = STATE.INI_TRI;
elseif current_state == STATE.CLOSE,
    ...
    switch task.type
        ...
        case 13 % Binocular rivalry task without reward and ITI states
            task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.SUCCESS STATE.ITI];
        case 14 % Binocular rivalry task without reward and ITI states ver2
            task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL];
            if current_state == 3
                task_states = [STATE.FIX_HOL];
                task.state_counter = task.state_counter - 1;
                if task.state_counter == 0;
                    task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL];
                end
            end
        case 15 % binocular rivalry task, (choose a unique number here :))
            task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.BIN_RIV_ACQ  STATE.BIN_RIV_HOL  STATE.SUCCESS  STATE.ITI];
    end
...
if task.type == 15
    global VAR
    if dyn.percept_switch == 0
        if ~VAR.playMovie && dyn.state~=2
            Screen(SETTINGS.window,'Flip');
        end
        
        % Movie-related
        dyn.movie_flip_counter = 1;
        % Create a keyboard queue for recording button press events
        KbQueueCreate();
        % Start recording keyboard events
        KbQueueStart();  
    end
    dyn.pseudo_memoryBuffer = [];
    dyn.counter_onset = 0;
...
while true
    ...
    [x_eye y_eye x_hnd y_hnd touching sen1 sen2 sen3 sen4] = aux_GetCalibratedEyeHandPos(task); % in deg
    if task.type == 15
        dyn.counter_onset = dyn.counter_onset + 1; 
        ...
        if VAR.condition(VAR.current_trial)==1 % F4 calibrates for fixation cue
            if any(find(keyCode)==115) && isfield(par_eye,'deg')% F4 
                fprintf(' Updating offsets x_eye = %.2f, y_eye =  %.2f \n', x_eye, y_eye );
                task.eye.offset_x = task.eye.offset_x - x_eye;
                task.eye.offset_y = task.eye.offset_y - y_eye;
            end
        end
        while any(find(keyCode)==115); [~, ~, keyCode] = KbCheck; end
    end
            ...
            for obj = 1:n_obj_eye
                if strcmp(SETTINGS.background_image, 'gratings')
                    if VAR.condition(VAR.current_trial)==1
                        ...
                        [is_within, fix_colour] = aux_IsWithinRadius_binoriv(x_eye, y_eye, par_eye(obj), dyn);
                        if is_within
                            success = true;
                            if isnan(dyn.target_selected(1)), dyn.target_selected(1) = obj; end
                        end
                        dyn.fix_colour = fix_colour;
                    end
                else
                    if aux_IsWithinRadius(x_eye, y_eye, par_eye(obj)),
                        success = true;
    ...
    if strcmp(SETTINGS.background_image, 'gratings') 
        if strcmp(fix_colour,'Red')
            fix_colour = 2;
        elseif strcmp(fix_colour,'Blue')
            fix_colour = 3;
        elseif strcmp(fix_colour,'Cue')
            fix_colour = 1;
        else
            fix_colour = 0;
        end
        ...
        if strcmp(SETTINGS.human_monkey, 'human') 
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue,fix_colour];
        elseif strcmp(SETTINGS.human_monkey, 'monkey') 
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,fix_colour];
        end
    else
    ...
    if GetSecs > tEnterState + dyn.duration    	% check time
        ...
        if task.type == 15
            ...
            KbQueueStop();
            % Flush the keyboard queue to reset it for the next movie
            KbQueueFlush();
        end
        ...
        fprintf(' aborted_effector %d  RT=%4d \n',dyn.aborted_effector, round((GetSecs-tEnterState)*1000));
        break
    end
    ...
    if task.type ==15
        if VAR.playMovie
            if VAR.condition(VAR.current_trial)~=1
                if GetSecs > tEnterState + VAR.flipInterval*dyn.movie_flip_counter
                    dyn.movie_flip_counter = dyn.movie_flip_counter + 1;
                    % Draw the next frame of the movie
                    tex = Screen('GetMovieImage', SETTINGS.window, VAR.movie);
                    if tex < 0
                        % Movie has ended, restart it
                        Screen('SetMovieTimeIndex', VAR.movie, 0);
                        tex = Screen('GetMovieImage', SETTINGS.window, VAR.movie);
                    end
                    if tex > 0
                        % Draw the texture
                        Screen('DrawTexture', SETTINGS.window, tex);

                        % Wait 1ms before updating the screen
                %         WaitSecs(0.001);

                        % Update the screen with the contents of the off-screen buffer
...
while true
    ...
    if task.type == 15
        ...
        [~, ~, keyCode] = KbCheck();
        if strcmp(SETTINGS.human_monkey, 'human') 
            % Check if the Alt button is being held down
            if any(find(keyCode)==18)
                % Alt button is being held down, set altButtonPressed to true
                repo_red = 1;
            else
                % Alt button is not being held down, set altButtonPressed to false
                repo_red = 0;
            end
            % Check if the Ctrl button is being held down
            if any(find(keyCode)==17)
                % Ctrl button is being held down, set ctrlButtonPressed to true
                repo_blue = 1;
            else
                % Ctrl button is not being held down, set ctrlButtonPressed to false
                repo_blue = 0;
            end
            if any(find(keyCode)==27) % Esc
                success = false;
            ...
            while any(find(keyCode)==115); [~, ~, keyCode] = KbCheck; end
            dyn.counterLine = dyn.counterLine+1;
            dyn.memoryBuffer(dyn.counterLine,1:13) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue];
        elseif strcmp(SETTINGS.human_monkey, 'monkey')
            if any(find(keyCode)==27) % Esc
                success = false;
                dyn.previousState = dyn.state;
            ...
            while any(find(keyCode)==115); [~, ~, keyCode] = KbCheck; end
            dyn.counterLine = dyn.counterLine+1;
            dyn.memoryBuffer(dyn.counterLine,1:11) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4];
        end
    else
        dyn.counterLine = dyn.counterLine+1;
        dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4];
        ...
        case 0 % eye
            ...
            for obj = start_obj_eye:n_obj_eye %obj = dyn.target_selected(1) %obj = 1:n_obj_eye Change 20140519
                if task.type == 15
                    ...
                    if strcmp(SETTINGS.human_monkey, 'human')
                        if strcmp(fix_colour,'Red')
                            dyn.memoryBuffer(dyn.counterLine,14) = 2;
                        elseif strcmp(fix_colour,'Blue')
                            dyn.memoryBuffer(dyn.counterLine,14) = 3;
                        elseif strcmp(fix_colour,'Cue')
                            dyn.memoryBuffer(dyn.counterLine,14) = 1;
                        else
                            dyn.memoryBuffer(dyn.counterLine,14) = 0;
                        end
                    elseif strcmp(SETTINGS.human_monkey, 'monkey')
                        if strcmp(fix_colour,'Red')
                            dyn.memoryBuffer(dyn.counterLine,12) = 2;
                        elseif strcmp(fix_colour,'Blue')
                            dyn.memoryBuffer(dyn.counterLine,12) = 3;
                        elseif strcmp(fix_colour,'Cue')
                            dyn.memoryBuffer(dyn.counterLine,12) = 1;
                        else
                            dyn.memoryBuffer(dyn.counterLine,12) = 0;
                        end
                    end
                    if ~is_within
                        if SETTINGS.allowBlinksOnly && ~aux_IsDisplacedDown(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius)
...
%                             dyn.state   = STATE.ABORT;
    ...
    elseif (dyn.state==STATE.FIX_HOL || dyn.state==STATE.FIX_ACQ) && task.type ~= 15
    ...
    if SETTINGS.AllowManualSkipping && mod(dyn.counterTimeSteps,100)
        [keyIsDown,~,keyCode] = KbCheck;
        if keyIsDown && ~any(find(keyCode)==80) && ~any(find(keyCode)==81)
...
while true
    ...
    if task.type == 15
        dyn.counter_onset = dyn.counter_onset + 1;
        if strcmp(SETTINGS.human_monkey, 'human') 
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,NaN,NaN,NaN];
        elseif strcmp(SETTINGS.human_monkey, 'monkey') 
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,NaN];
        end
    else
        dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4];
    end
    ...
    if task.type == 15
        if (GetSecs - tEnterState) > flip_timing(flip_counter) % flip every 4 sec to avoid automatic flipping
            Screen(SETTINGS.window,'Flip');
...
if strcmp(SETTINGS.background_image, 'gratings')
    global VAR
%     dyn.trialNumber = VAR.current_trial;
end
if ~dyn.(['n_' eff '_' pha])==0
    for obj=1:numel(trial(dyn.trialNumber).(eff).(pha))
        ...
        %% color!
        if strcmp(SETTINGS.background_image, 'gratings')
            if strcmp(pha, 'fix') || strcmp(pha, 'fi2') || strcmp(pha, 'cue')
                % 1=fixation, 2=red, 3=blue, 4=binoriv, 5=rest, 6=long break
                if VAR.condition(VAR.current_trial)==1
                    RGB_color=[0 0 0];
                    if strcmp(eff,'hnd')
                        RGB_color=dyn.hnd_color;
                    end
                    if ismember(obj,dyn.correct_choice_target)
                        plot(0,0,'*','color',[0 0 0],'Tag',[eff pha 'correct'],'Visible','off');
                    end
                    plot(x1,y1,'color',RGB_color,'Tag',[eff pha 'Size'],'Visible','off');
                    plot(x2,y2,'color',RGB_color,'linestyle',':','Tag',[eff pha 'Radius'],'Visible','off');
                elseif VAR.condition(VAR.current_trial)==2
                    RGB_color_red=[255 0 0]/255;
                    if strcmp(eff,'hnd')
                        RGB_color_red=dyn.hnd_color/255;
                    end
                    if ismember(obj,dyn.correct_choice_target)
                        plot(x.red,y.red,'*','color',[1 0 0],'Tag',[eff pha 'correct'],'Visible','off');
                        hold on
                    end
                    plot(x.red+x1,y.red+y1,'color',RGB_color_red,'Tag',[eff pha 'Size'],'Visible','off');
                    plot(x.red+x2,y.red+y2,'color',RGB_color_red,'linestyle',':','Tag',[eff pha 'Radius'],'Visible','off');
                elseif VAR.condition(VAR.current_trial)==3
                    RGB_color_blue=[0 0 255]/255;
                    if strcmp(eff,'hnd')
                        RGB_color_blue=dyn.hnd_color/255;
                    end
                    if ismember(obj,dyn.correct_choice_target)
                        plot(x.blue,y.blue,'*','color',[0 0 1],'Tag',[eff pha 'correct'],'Visible','off');
                        hold on
                    end
                    plot(x.blue+x1,y.blue+y1,'color',RGB_color_blue,'Tag',[eff pha 'Size'],'Visible','off');
                    plot(x.blue+x2,y.blue+y2,'color',RGB_color_blue,'linestyle',':','Tag',[eff pha 'Radius'],'Visible','off');
                elseif VAR.condition(VAR.current_trial)==4
                    RGB_color_red=[255 0 0]/255;
                    RGB_color_blue=[0 0 255]/255;
                    if strcmp(eff,'hnd')
...
is_down = 0;
if strcmp(SETTINGS.background_image, 'gratings')
    if abs(x0(1,1) - x) < r && y < y0(1,1)
        is_down = 1;
    elseif abs(x0(2,1) - x) < r && y < y0(2,1)
        is_down = 1;
    end
else
    if abs(x0 - x) < r && y < y0
...
if strcmp(SETTINGS.background_image, 'gratings')
    global VAR
    stim=[par_temp.x.red par_temp.y.red par_temp.x.blue par_temp.y.blue par_temp.size par_temp.radius];

    % keep the values in deg for experimenter GUI plotting
    par.deg.x(1,1)              = stim(1); % red
    par.deg.x(2,1)              = stim(3); % blue 
    par.deg.y(1,1)              = stim(2); % red
    par.deg.y(2,1)              = stim(4); % blue
    par.deg.size           = stim(5);
    par.deg.radius         = stim(6);

    % convert to pixels for PTB display, adjuted for red
    x_rad = stim(1)*pi/180;
    y_rad = stim(3)*pi/180;
%     par.offset_deg = atan(sqrt(tan(x_rad)^2 + tan(y_rad)^2));
    par.offset_deg = 0;
    stim(5)= deg2pix_withOffset (stim(5),par.offset_deg);
    stim(6)= deg2pix_withOffset (stim(5),par.offset_deg);

    [stim(1) stim(2)] = deg2pix_xy(stim(1), stim(2)); % red
    [stim(3) stim(4)] = deg2pix_xy(stim(3), stim(4)); % blue

    par.pix.x(1,1)              = stim(1); % red
    par.pix.x(2,1)              = stim(3); % blue 
    par.pix.y(1,1)              = stim(2); % red
    par.pix.y(2,1)              = stim(4); % blue
    par.pix.size           = stim(5);
    par.stimRect(:,1)       = aux_pr2rect([stim(1)  stim(2)]', stim(5)); % red
    par.stimRect(:,2)       = aux_pr2rect([stim(3)  stim(4)]', stim(5)); % blue
    par.x(1,1)              = stim(1);% red
    par.x(2,1)              = stim(3);% blue 
    par.y(1,1)              = stim(2);% red
    par.y(2,1)              = stim(4);% blue 
        ...
        if size(par.stimColor,1)==2 %% hand selection targets, 20140603
            if strcmp(SETTINGS.background_image, 'gratings')
                ...
                if VAR.condition(VAR.current_trial)==1
                    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
                    Screen('FillRect', SETTINGS.window, [186 186 186]); 
                    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 0]);
                    Screen('FillOval', SETTINGS.window, [0 0 0], [SETTINGS.screen_w_pix/2-VAR.radius_fp SETTINGS.screen_h_pix/2-VAR.radius_fp SETTINGS.screen_w_pix/2+VAR.radius_fp SETTINGS.screen_h_pix/2+VAR.radius_fp]);
                    Screen(SETTINGS.window,'Flip');
                end
            else
...
function combine_trials_from_single_matfiles(folder,subfolder,task)
global SETTINGS
...
if task.type == 15
    global VAR
    if exist('trial','var')
        save([folder filesep subfolder],'SETTINGS','task','trial','VAR');
    end
    ...
    if exist('trial','var')
        save([SETTINGS.dag_drive folder(3:end) filesep subfolder],'SETTINGS','task','trial','VAR');
    end
else
    if exist('trial','var')
...
function binoriv_stimuli(condition, stimulus)
global VAR
global SETTINGS
...
% condition: 1=fixation, 2=red, 3=blue, 4=binoriv, 5=rest, 6=long break
if condition == 1
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
    Screen('FillOval', SETTINGS.window, [0 0 0], [SETTINGS.screen_w_pix/2-VAR.radius_fp SETTINGS.screen_h_pix/2-VAR.radius_fp SETTINGS.screen_w_pix/2+VAR.radius_fp SETTINGS.screen_h_pix/2+VAR.radius_fp]);
    fprintf('\nFixation cue presented...\n')
elseif condition == 2
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
    face = Screen('MakeTexture', SETTINGS.window, stimulus);
	Screen('DrawTexture', SETTINGS.window, face)
elseif condition == 3
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
    obj = Screen('MakeTexture', SETTINGS.window, stimulus);
	Screen('DrawTexture', SETTINGS.window, obj)
elseif condition == 4
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
    bino = Screen('MakeTexture', SETTINGS.window, stimulus);
...
%     imdata_bino  = imread('BinoRiv_stimuli/without_fp/Face_1_Object_1.tif', 'tif'); % binocular image 
%     bino = Screen('MakeTexture', SETTINGS.window, imdata_bino);
% 	Screen('DrawTexture', SETTINGS.window, bino)
end


function [is_within, fix_colour] = aux_IsWithinRadius_binoriv(x,y,par,dyn)
global SETTINGS VAR
%% check if (x,y) is within radius r centered on (x0,y0)
if strcmp(SETTINGS.background_image, 'gratings')
    x0=par.deg.x;
    ...
    is_within = 0;
    fix_colour = '';
    if VAR.condition(VAR.current_trial) == 1 % fixation cue
        switch par.radiusShape
            case 'circle'
                if sqrt(x^2 + y^2) < r
                    is_within = 1;
    ...
    elseif VAR.condition(VAR.current_trial) == 4 % binocular rivalry
        switch par.radiusShape
            case 'circle'
                if sqrt(((x0(1,1) - x))^2 + (y0(1,1) - y)^2) < r
                    is_within = 1;
                    fix_colour = 'Red';
                elseif sqrt(((x0(2,1) - x))^2 + (y0(2,1) - y)^2) < r
                    is_within = 1;
                    fix_colour = 'Blue';
                else
                    fix_colour = 'None';
    ...
    elseif VAR.condition(VAR.current_trial) == 2 % red
        switch par.radiusShape
            case 'circle'
                if sqrt(((x0(1,1) - x))^2 + (y0(1,1) - y)^2) < r
                    is_within = 1;
    ...
    elseif VAR.condition(VAR.current_trial) == 3 % blue
        switch par.radiusShape
            case 'circle'
                if sqrt(((x0(2,1) - x))^2 + (y0(2,1) - y)^2) < r
                    is_within = 1;
                    fix_colour = 'Blue';
                else
                    fix_colour = 'None';
...
function conditions = create_conditions()
global VAR
VAR.num_trial_phys = 8;
VAR.num_trial_bino = 1;
VAR.num_triad = 12;
VAR.num_superblock = 100;%5; % the number of super-blocks
VAR.short_break = 8; % break time (sec) after physical/binocular blocks % 8000 ms
VAR.long_break = 8; % break time (sec) after one super-block 

VAR.only_binoriv = 0; % 1 if you want to show only binocular rivalry stimlus
VAR.Skip_IfFailure = 0; % 1: skip the rest of the trials in a block if fixation failure
VAR.playMovie = false;
VAR.fix_trial_duration = true;

VAR.current_trial = 0;
VAR.condition = []; % 1=fixation, 2=red, 3=blue, 4=binoriv, 5=rest, 6=long break
% For the physical switches 
VAR.phys_colour = [];
if VAR.num_trial_phys < 2
    VAR.phys_colour = randi([0 1],1,1); 
end
for i=1:VAR.num_trial_phys/2;VAR.phys_colour=horzcat(VAR.phys_colour,0);end
for i=1:VAR.num_trial_phys/2;VAR.phys_colour=horzcat(VAR.phys_colour,1);end
...
for spb = 1:VAR.num_superblock
    for trd = 1:VAR.num_triad
        ...
        VAR.phys_colour = VAR.phys_colour(randperm(VAR.num_trial_phys)); % shuffle the order of the phys stimuli
        if switch_seed == 0 % phys -> binoriv
            for trl = 1:VAR.num_trial_phys
...
function [trial, trials_store, triad_trial] = BinoRiv_ITI_saving(trial,task,dyn,trials_store)
global VAR SETTINGS
%% Store data for ITI trial
trial(dyn.trialNumber).states = [trial(dyn.trialNumber).states dyn.state];
trial(dyn.trialNumber).states_onset = [trial(dyn.trialNumber).states_onset dyn.states_onset];
trial(dyn.trialNumber).chopping_label = [trial(dyn.trialNumber).chopping_label dyn.percept_switch];
trial(dyn.trialNumber).counter_onset = [trial(dyn.trialNumber).counter_onset dyn.counter_onset];
...
trial(dyn.trialNumber).counter=dyn.counterLine;
if strcmp(SETTINGS.human_monkey, 'human')
    trial(dyn.trialNumber).repo_red = dyn.memoryBuffer(:,12);
    trial(dyn.trialNumber).repo_blue = dyn.memoryBuffer(:,13);
    trial(dyn.trialNumber).fix_colour_sample = dyn.memoryBuffer(:,14);
elseif strcmp(SETTINGS.human_monkey, 'monkey')
    trial(dyn.trialNumber).fix_colour_sample = dyn.memoryBuffer(:,12);
end


%% Chop the trial(i.e.triad) into real trials
% dyn.trialNumber is 'triad' number in this task
if VAR.num_trial_phys == VAR.num_trial_bino
    triad_trial = (dyn.trialNumber-1) * ((VAR.num_trial_bino)*2 + 2);
else
    triad_trial = (dyn.trialNumber-1) * ((VAR.num_trial_phys+VAR.num_trial_bino) + 2);
end

% split fixation cue trial
fix_cue_label = find(trial(dyn.trialNumber).states == 33);
fix_cue_label = fix_cue_label(1) - 1;

% triad based
trials_store(triad_trial+1).timestamp = trial(dyn.trialNumber).timestamp;
...
trials_store(triad_trial+1).type = trial(dyn.trialNumber).type;
trials_store(triad_trial+1).effector = trial(dyn.trialNumber).effector;
...
trials_store(triad_trial+1).completed = trial(dyn.trialNumber).completed;
trials_store(triad_trial+1).aborted_state = trial(dyn.trialNumber).aborted_state;
trials_store(triad_trial+1).aborted_state_duration = trial(dyn.trialNumber).aborted_state_duration;
trials_store(triad_trial+1).abort_code = trial(dyn.trialNumber).abort_code;
% trial based
trials_store(triad_trial+1).n = triad_trial+1;
trials_store(triad_trial+1).success = trial(dyn.trialNumber).success(1);
trials_store(triad_trial+1).stimulus = trial(dyn.trialNumber).stimulus(1);
trials_store(triad_trial+1).eye = trial(dyn.trialNumber).eye(1);
trials_store(triad_trial+1).hnd = trial(dyn.trialNumber).hnd(1);
trials_store(triad_trial+1).reward_size = trial(dyn.trialNumber).reward_size;
trials_store(triad_trial+1).task = trial(dyn.trialNumber).task;
trials_store(triad_trial+1).target_selected = trial(dyn.trialNumber).target_selected(1);
trials_store(triad_trial+1).target2_selected = trial(dyn.trialNumber).target2_selected(1);
trials_store(triad_trial+1).reward_selected = trial(dyn.trialNumber).reward_selected(1);
trials_store(triad_trial+1).reward_prob = trial(dyn.trialNumber).reward_prob(1);
trials_store(triad_trial+1).reward_time = trial(dyn.trialNumber).reward_time(1);
% state based
trials_store(triad_trial+1).states = trial(dyn.trialNumber).states(1:fix_cue_label);
trials_store(triad_trial+1).states_onset = trial(dyn.trialNumber).states_onset(1:fix_cue_label);
trials_store(triad_trial+1).fix_colour = trial(dyn.trialNumber).fix_colour(1:fix_cue_label);
trials_store(triad_trial+1).chopping_label = trial(dyn.trialNumber).chopping_label(1:fix_cue_label);
onset_counts = sum(trial(dyn.trialNumber).counter_onset(1:fix_cue_label));
% sample based
trials_store(triad_trial+1).tSample_from_time_start = trial(dyn.trialNumber).tSample_from_time_start(1:onset_counts);
trials_store(triad_trial+1).state = trial(dyn.trialNumber).state(1:onset_counts);
trials_store(triad_trial+1).x_hnd = trial(dyn.trialNumber).x_hnd(1:onset_counts);
trials_store(triad_trial+1).y_hnd = trial(dyn.trialNumber).y_hnd(1:onset_counts);
trials_store(triad_trial+1).x_eye = trial(dyn.trialNumber).x_eye(1:onset_counts);
trials_store(triad_trial+1).y_eye = trial(dyn.trialNumber).y_eye(1:onset_counts);
trials_store(triad_trial+1).sen_L = trial(dyn.trialNumber).sen_L(1:onset_counts);
trials_store(triad_trial+1).sen_R = trial(dyn.trialNumber).sen_R(1:onset_counts);
trials_store(triad_trial+1).jaw = trial(dyn.trialNumber).jaw(1:onset_counts);
trials_store(triad_trial+1).counter = onset_counts;
if strcmp(SETTINGS.human_monkey, 'human')
    trials_store(triad_trial+1).repo_red = trial(dyn.trialNumber).repo_red(1:onset_counts);
    trials_store(triad_trial+1).repo_blue = trial(dyn.trialNumber).repo_blue(1:onset_counts);
end
trials_store(triad_trial+1).fix_colour_sample = trial(dyn.trialNumber).fix_colour_sample(1:onset_counts);

% split main trials
main_label = [];
ite = 0;
loop = 0;
if any(find(trial(dyn.trialNumber).states==20))
    max_main_trials = find(trial(dyn.trialNumber).states==20);
else
    max_main_trials = find(trial(dyn.trialNumber).states==50);
end
for label = (fix_cue_label+1):max_main_trials-1
    if (trial(dyn.trialNumber).states(label) == 34 && trial(dyn.trialNumber).chopping_label(label) == 0 && trial(dyn.trialNumber).states(label+1) ~= 19) || (trial(dyn.trialNumber).states(label) == 19)
        ite = ite + 1;
        % mono-value based
        trials_store(triad_trial+1+ite).timestamp = trial(dyn.trialNumber).timestamp;
        ...
        trials_store(triad_trial+1+ite).type = trial(dyn.trialNumber).type;
        trials_store(triad_trial+1+ite).effector = trial(dyn.trialNumber).effector;
        ...
        trials_store(triad_trial+1).completed = trial(dyn.trialNumber).completed;
        trials_store(triad_trial+1+ite).aborted_state = trial(dyn.trialNumber).aborted_state;
        trials_store(triad_trial+1+ite).aborted_state_duration = trial(dyn.trialNumber).aborted_state_duration;
        trials_store(triad_trial+1+ite).abort_code = trial(dyn.trialNumber).abort_code;
        % trial based
        trials_store(triad_trial+1+ite).n = triad_trial+1+ite;
        trials_store(triad_trial+1+ite).success = trial(dyn.trialNumber).success(ite+1); % +1 is considering the 1st fixation trial 
        trials_store(triad_trial+1+ite).stimulus = trial(dyn.trialNumber).stimulus(ite+1);
        trials_store(triad_trial+1+ite).eye = trial(dyn.trialNumber).eye(ite+1);
        trials_store(triad_trial+1+ite).hnd = trial(dyn.trialNumber).hnd(ite+1);
        trials_store(triad_trial+1+ite).reward_size = trial(dyn.trialNumber).reward_size;
        trials_store(triad_trial+1+ite).task = trial(dyn.trialNumber).task; 
        trials_store(triad_trial+1+ite).target_selected = trial(dyn.trialNumber).target_selected(ite+1);
        trials_store(triad_trial+1+ite).target2_selected = trial(dyn.trialNumber).target2_selected(ite+1);
        trials_store(triad_trial+1+ite).reward_selected = trial(dyn.trialNumber).reward_selected(ite+1);
        trials_store(triad_trial+1+ite).reward_prob = trial(dyn.trialNumber).reward_prob(ite+1);
        trials_store(triad_trial+1+ite).reward_time = trial(dyn.trialNumber).reward_time(ite+1);
        % state based
        loop = 0;
        while true % find the trial-started label
            loop = loop + 1;
            if trial(dyn.trialNumber).states(label-loop) == 33 && trial(dyn.trialNumber).chopping_label(label-loop) == 0
                trial_start_label = label-loop;
                break
            end
        end
        trials_store(triad_trial+1+ite).states = trial(dyn.trialNumber).states(trial_start_label:label);
        trials_store(triad_trial+1+ite).states_onset = trial(dyn.trialNumber).states_onset(trial_start_label:label);
        trials_store(triad_trial+1+ite).fix_colour = trial(dyn.trialNumber).fix_colour(trial_start_label:label);
        trials_store(triad_trial+1+ite).chopping_label = trial(dyn.trialNumber).chopping_label(trial_start_label:label);
        onset_counts = sum(trial(dyn.trialNumber).counter_onset(1:label));
        pre_onset_counts = sum(trial(dyn.trialNumber).counter_onset(1:trial_start_label-1))+1;
        % sample based
        trials_store(triad_trial+1+ite).tSample_from_time_start = trial(dyn.trialNumber).tSample_from_time_start(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).state = trial(dyn.trialNumber).state(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).x_hnd = trial(dyn.trialNumber).x_hnd(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).y_hnd = trial(dyn.trialNumber).y_hnd(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).x_eye = trial(dyn.trialNumber).x_eye(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).y_eye = trial(dyn.trialNumber).y_eye(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).sen_L = trial(dyn.trialNumber).sen_L(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).sen_R = trial(dyn.trialNumber).sen_R(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).jaw = trial(dyn.trialNumber).jaw(pre_onset_counts:onset_counts);
        trials_store(triad_trial+1+ite).counter = onset_counts - pre_onset_counts + 1;
        if strcmp(SETTINGS.human_monkey, 'human')
            trials_store(triad_trial+1+ite).repo_red = trial(dyn.trialNumber).repo_red(pre_onset_counts:onset_counts);
            trials_store(triad_trial+1+ite).repo_blue = trial(dyn.trialNumber).repo_blue(pre_onset_counts:onset_counts);
        end
        trials_store(triad_trial+1+ite).fix_colour_sample = trial(dyn.trialNumber).fix_colour_sample(pre_onset_counts:onset_counts);
    end
end

% split resting/ITI trial
if find(trial(dyn.trialNumber).states == 50)
    rest_label = find(trial(dyn.trialNumber).states == 50);
    ite = ite + 1;
    % mono-value based
    trials_store(VAR.current_trial).timestamp = trial(dyn.trialNumber).timestamp;
    trials_store(VAR.current_trial).force_hand = NaN;
    trials_store(VAR.current_trial).manual_success = NaN;
    trials_store(VAR.current_trial).force_small_reward = NaN;
    trials_store(VAR.current_trial).microstim = NaN;
    trials_store(VAR.current_trial).microstim_interval = NaN;
    trials_store(VAR.current_trial).microstim_state = NaN;
    trials_store(VAR.current_trial).microstim_start = NaN;
    trials_store(VAR.current_trial).microstim_end = NaN;
    trials_store(VAR.current_trial).type = trial(dyn.trialNumber).type;
    trials_store(VAR.current_trial).effector = trial(dyn.trialNumber).effector;
    trials_store(VAR.current_trial).rest_hand = NaN;
    trials_store(VAR.current_trial).reach_hand = NaN;
    trials_store(VAR.current_trial).hand_choice = NaN;
    trials_store(VAR.current_trial).choice = NaN;
    trials_store(VAR.current_trial).reward_modulation = NaN;
    trials_store(VAR.current_trial).small_reward = NaN;
    trials_store(VAR.current_trial).CueAuditiv = NaN;
    trials_store(dyn.trialNumber).completed = trial(dyn.trialNumber).completed;
    trials_store(VAR.current_trial).aborted_state = trial(dyn.trialNumber).aborted_state;
    trials_store(VAR.current_trial).aborted_state_duration = trial(dyn.trialNumber).aborted_state_duration;
    trials_store(VAR.current_trial).abort_code = trial(dyn.trialNumber).abort_code;
    % trial based
    trials_store(VAR.current_trial).n = VAR.current_trial;
    trials_store(VAR.current_trial).success = trial(dyn.trialNumber).success(ite+1); % +1 is considering the 1st fixation trial 
    trials_store(VAR.current_trial).stimulus = trial(dyn.trialNumber).stimulus(ite+1);
    trials_store(VAR.current_trial).eye = trial(dyn.trialNumber).eye(ite+1);
    trials_store(VAR.current_trial).hnd = trial(dyn.trialNumber).hnd(ite+1);
    trials_store(VAR.current_trial).reward_size = trial(dyn.trialNumber).reward_size;
    trials_store(VAR.current_trial).task = trial(dyn.trialNumber).task;
    trials_store(VAR.current_trial).target_selected = trial(dyn.trialNumber).target_selected(ite+1);
    trials_store(VAR.current_trial).target2_selected = trial(dyn.trialNumber).target2_selected(ite+1);
    trials_store(VAR.current_trial).reward_selected = trial(dyn.trialNumber).reward_selected(ite+1);
    trials_store(VAR.current_trial).reward_prob = trial(dyn.trialNumber).reward_prob(ite+1);
    trials_store(VAR.current_trial).reward_time = trial(dyn.trialNumber).reward_time(ite+1);
    % state based
    trials_store(VAR.current_trial).states = trial(dyn.trialNumber).states(rest_label);
    trials_store(VAR.current_trial).states_onset = trial(dyn.trialNumber).states_onset(rest_label);
    trials_store(VAR.current_trial).fix_colour = {'NaN'};
    trials_store(VAR.current_trial).chopping_label = 0;
    onset_counts = sum(trial(dyn.trialNumber).counter_onset(1:rest_label));
    end_triad_counter = sum(trial(dyn.trialNumber).counter_onset(1:rest_label-1));
    % sample based
    trials_store(VAR.current_trial).state = NaN;
    trials_store(VAR.current_trial).state = NaN;
    trials_store(VAR.current_trial).x_hnd = NaN;
    trials_store(VAR.current_trial).y_hnd = NaN;
    trials_store(VAR.current_trial).x_eye = NaN;
    trials_store(VAR.current_trial).y_eye = NaN;
    trials_store(VAR.current_trial).sen_L = NaN;
    trials_store(VAR.current_trial).sen_R = NaN;
    trials_store(VAR.current_trial).jaw = NaN;
    trials_store(VAR.current_trial).counter = NaN;
    if strcmp(SETTINGS.human_monkey, 'human')
        trials_store(VAR.current_trial).repo_red = NaN;
        trials_store(VAR.current_trial).repo_blue = NaN;
    end
    trials_store(VAR.current_trial).fix_colour_sample = NaN;
end
