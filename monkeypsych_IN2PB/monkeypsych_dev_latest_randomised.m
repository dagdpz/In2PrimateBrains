...
for spb = 1:VAR.num_superblock
    for trd = 1:VAR.num_triad
        ...
        % Main switches
        randmised_cond = [];
        for i=1:VAR.num_trial_bino
            randmised_cond = vertcat(randmised_cond, 4);
        end
        for i=1:VAR.num_trial_phys/2
            randmised_cond = vertcat(randmised_cond, 2);
        end
        for i=1:VAR.num_trial_phys/2
            randmised_cond = vertcat(randmised_cond, 3);
        end
        randmised_cond = randmised_cond(randperm(VAR.num_trial_bino+VAR.num_trial_phys)); % shuffle the order of the phys stimuli
        VAR.condition = vertcat(VAR.condition, randmised_cond);
