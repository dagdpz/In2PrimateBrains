function monkeypsych_dev_binoriv(subject)
...
[task monkey_name DATA_PATH]  = get_monkey_dev(subject);
SETTINGS.vd         = task.vd;
sequence_indexes    = 0;
    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            if task.type == 15
                ...
                task.eye.fix.pos=[task.eye.fix.x.red task.eye.fix.y.red task.eye.fix.size task.eye.fix.radius 1; task.eye.fix.x.blue task.eye.fix.y.blue task.eye.fix.size task.eye.fix.radius 1]; %% legacy for analysis
                task.hnd.fix.pos=[task.hnd.fix.x.red task.hnd.fix.y.red task.hnd.fix.size task.hnd.fix.radius 1; task.hnd.fix.x.blue task.hnd.fix.y.blue task.hnd.fix.size task.hnd.fix.radius 1]; %% legacy for analysis
            else
...
%                 dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
            ...
            if task.type == 15
                if success == 0
                    ...
                    trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye];
                    trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd];
...
%             dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
            ...
            if success == 0
                ...
                trial(dyn.trialNumber).eye = [trial(dyn.trialNumber).eye task.eye]; % Info about fixation points
                trial(dyn.trialNumber).hnd = [trial(dyn.trialNumber).hnd task.hnd];
%                 trial(dyn.trialNumber).reward_size = [trial(dyn.trialNumber).reward_size trial(dyn.trialNumber).reward_size]; 
%                 trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
                % state base
...
%                 trial(dyn.trialNumber).counter=dyn.counterLine; 
        ...
        case STATE.CLOSE % close gracefully
            ...
            if isfield(trial,'state') && ~isempty(trial(1).state)
                ...
                combine_trials_from_single_matfiles(pathname,task,filename);
...
function combine_trials_from_single_matfiles(folder,task,subfolder)
global SETTINGS
