...
%                 Screen('DrawTexture', SETTINGS.window, bino)
                ...
                FP_loc = 0; % 0: fixed distance (theta) from the centre, 1: FP onto non-grating-overlapped place, 2: FP onto grating-overlapped place
                distfromcent = 1; % distance from centre in num of grating lines; integer
                theta = 2.0; % distance of a fixation spot from the centre (deg)
...
task.timing.fix_time_to_acquire_eye     = 1; % duration of STATE.FIX_ACQ
task.timing.tar_time_to_acquire_eye     = 0; % duration of STATE.FIX_ACQ
task.timing.tar_inv_time_to_acquire_eye = 0;
task.timing.fix_time_hold               = 2; % duration of FIX_HOL
...
task.timing.tar_time_to_acquire_eye = 2; % duration of STATE.BIN_RIV_ACQ (allowed time of fixing/switching gaze)
